...
def update_active_calls(n_intervals, pathname, previous_calls_data):
    ...
    # Components are now persistent (just hidden with CSS), so no pathname check needed
    # This allows the dropdown to update even when agent page is hidden
    ...
    # ALSO treat initial calls (triggered_id is None) as returning to page
    # This fixes the bug where dropdown isn't populated after page recreation
    is_returning_to_agent_page = (triggered_id == 'url' and pathname in ('/', '/agent'))
    is_initial_call = (triggered_id is None or not ctx.triggered)

    if is_returning_to_agent_page or is_initial_call:
        print(f"🔄 [ACTIVE_CALLS] Returning to agent page or initial call, fetching calls...")
    ...
    try:
        ...
        # BUT: Always update if returning to agent page OR initial call (to ensure dropdown is populated)
        # This is critical for restoring state when navigating back or when component is recreated
        if not is_returning_to_agent_page and not is_initial_call and previous_calls_data:
...
def update_selected_call_id(selected_call_id, pathname, current_stored_id):
    ...
    if ctx.triggered:
        ...
        if triggered_id == 'url':
            # Preserve the stored call ID always (pages are persistent now)
...
def update_main_content(selected_call_id, pathname):
    ...
    # CRITICAL FIX: Don't recreate content on pathname changes
    # Components are now persistent, recreating them destroys their state
    if triggered_id == 'url':
        # Pathname changed - don't recreate content, it's already there
        raise PreventUpdate

    # Only update when call selection changes
...
# Create agent layout function (to be called once)
def create_agent_layout():
    """Create agent layout - called once at startup"""
...
# Routing callback - show/hide pages with CSS instead of recreating
@app.callback(
...
def display_page(pathname, current_children):
    """Route to different pages - maintain persistent layouts"""
    # On first load, create all three layouts
    if current_children is None or not current_children:
        return html.Div([
            html.Div(create_agent_layout(), id='agent-page', style={'display': 'block'}),
            html.Div(create_supervisor_layout(), id='supervisor-page', style={'display': 'none'}),
            html.Div(create_analytics_layout(), id='analytics-page', style={'display': 'none'})
        ])

    # After initial load, just update visibility - DON'T recreate layouts
    raise PreventUpdate

# New callback to control page visibility
@app.callback(
    Output('agent-page', 'style'),
    Output('supervisor-page', 'style'),
    Output('analytics-page', 'style'),
    Input('url', 'pathname')
)
def update_page_visibility(pathname):
    """Show/hide pages based on URL - preserves state"""
