    ...
    try:
        ...
        for attempt in range(max_attempts):
            ...
            if status.status.state == StatementState.SUCCEEDED:
                ...
                st.error(f"Query failed: {status.status.state_message}")
                return pd.DataFrame()
...
with tab2:
    ...
    if not top_agents.empty:
        ...
        with col1:
            ...
            if 'performance_score' in top_agents.columns:
                st.bar_chart(top_agents.set_index('agent_id')['performance_score'])
        
        with col2:
            st.subheader("Compliance Rate")
            if 'compliance_rate' in top_agents.columns:
                st.bar_chart(top_agents.set_index('agent_id')['compliance_rate'])
    else:
...
with tab4:
    ...
    if not daily_stats.empty:
        ...
        with col1:
            ...
            if 'call_date' in daily_stats.columns and 'total_calls' in daily_stats.columns:
                daily_stats['call_date'] = pd.to_datetime(daily_stats['call_date'])
                st.line_chart(daily_stats.set_index('call_date')['total_calls'])
        
        with col2:
            st.subheader("Positive Sentiment Rate")
            if 'call_date' in daily_stats.columns and 'positive_sentiment_rate' in daily_stats.columns:
                daily_stats['call_date'] = pd.to_datetime(daily_stats['call_date'])
                st.line_chart(daily_stats.set_index('call_date')['positive_sentiment_rate'])
